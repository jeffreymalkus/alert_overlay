...
Sections:
  ...
  2. FLR STANDALONE
  3. COMBINED CLEAN STACK (BDR + FLR, unconstrained)
  4. COMBINED CLEAN STACK CAPPED (max 3 concurrent)

  RETIRED: SC LONG (PF 0.48), EMA PULL SHORT (N=26, shelved), EMA FPIP (N=2, dead)
...
def _flr_cfg() -> OverlayConfig:
    """FLR only — promoted config (4-bar turn + stop=0.50)."""
    cfg = _base_cfg()
    cfg.show_fl_momentum_rebuild = True
    # Uses config defaults: flr_turn_confirm_bars=4, flr_stop_frac=0.50
    return cfg


def _all_promoted_cfg() -> OverlayConfig:
    """All promoted setups combined: BDR Short + FLR."""
    cfg = _base_cfg()
    # BDR Short
    cfg.show_breakdown_retest = True
    cfg.bdr_require_red_trend = True
...
def main():
    print("=" * 150)
    print("PROMOTED STRATEGY STACK — Clean Stack Validation Replay")
    print("Stack: BDR Short + FLR  |  RETIRED: SC Long, EMA Pull, EMA FPIP")
    ...
    # ══════════════════════════════════════════════════════════
    #  SECTION 1: BDR SHORT STANDALONE
    # ══════════════════════════════════════════════════════════
    print("\n" + "=" * 150)
    print("SECTION 1 — BDR SHORT STANDALONE")
    print("=" * 150)
    ...
    # ══════════════════════════════════════════════════════════
    #  SECTION 2: FLR STANDALONE
    # ══════════════════════════════════════════════════════════
    print("\n" + "=" * 150)
    print("SECTION 2 — FLR STANDALONE (4-bar turn + stop=0.50)")
    print("=" * 150)

    print("\n  Running FLR standalone...")
    cfg_flr = _flr_cfg()
    flr_trades = _run_all_symbols(symbols, cfg_flr, spy_bars, qqq_bars, sector_bars_dict,
                                   setup_filter={SetupId.FL_MOMENTUM_REBUILD})
    print(HEADER)
    print(DIVIDER)
    print(fmt_row("FLR (4-bar turn, stop=0.50)", compute_metrics(flr_trades)))

    m, v = _print_setup_summary("FL_MOMENTUM_REBUILD", flr_trades, date_range, n_symbols)
    verdicts["FLR"] = v
    _print_monthly("FLR", flr_trades)
    _export_csv(flr_trades, OUT_DIR / "replay_flr_standalone.csv", "flr")
    ...
    # ══════════════════════════════════════════════════════════
    #  SECTION 3: COMBINED CLEAN STACK (unconstrained)
    # ══════════════════════════════════════════════════════════
    print("\n" + "=" * 150)
    print("SECTION 3 — COMBINED CLEAN STACK (BDR + FLR, unconstrained)")
    print("=" * 150)

    print("\n  Running combined clean stack...")
    cfg_all = _all_promoted_cfg()
    promoted_ids = {SetupId.BDR_SHORT, SetupId.FL_MOMENTUM_REBUILD}
    combined_trades = _run_all_symbols(symbols, cfg_all, spy_bars, qqq_bars,
                                        sector_bars_dict, setup_filter=promoted_ids)
    ...
    # ══════════════════════════════════════════════════════════
    #  SECTION 4: COMBINED CLEAN STACK CAPPED (max 3)
    # ══════════════════════════════════════════════════════════
    print("\n" + "=" * 150)
    print("SECTION 4 — COMBINED CLEAN STACK CAPPED (max 3 concurrent)")
    print("=" * 150)
    ...
    print("\n  RETIRED setups: SC Long (PF 0.48), EMA Pull Short (N=26), EMA FPIP (N=2)")
